...
- 螢幕與電腦一多、而且不是每次都一起切換時，可以把會一起使用的編成群組。選了某個群組，切換中心、主機切換器與系統匣就只顯示它涵蓋的範圍。群組只存在這台電腦，不會同步給已配對的主機。
- 系統匣（macOS 為選單列）也能直接切換：把全部螢幕切到某台主機，或個別切換每台螢幕。

**螢幕維護（重送訊號為實驗性功能）**

- 畫面在正確的輸入上卻沒有出來時，可以把螢幕切到另一台主機的輸入再切回來，讓它重新接上這台電腦。螢幕內建的 USB／KVM 綁在目前作用中的輸入上，所以鍵盤滑鼠也會一起回來。
- 畫面離開這台電腦之後，只有當下在畫面上的那台主機能把它切回來。MuxSU 會先確認對方有回應才動手，沒有回應就完全不動螢幕。這項功能需要在設定的「實驗性功能」分頁開啟，預設不顯示。
- 重新偵測會再向螢幕讀一次它接受哪些輸入、目前顯示哪一個；讀不到時保留原本的設定。它只做讀取，不屬於實驗性。
...
可以直接看到每台共用螢幕目前交給哪台電腦，以及各自使用的輸入；螢幕畫面會染上正在顯示的主機顏色。共用螢幕不只一台時，上方會出現把全部螢幕一次切換過去的按鈕。建立群組之後，標題下方會多一列群組，選了某一組就只顯示它涵蓋的螢幕與電腦。

![MuxSU 切換中心，上方一列群組，兩台共用螢幕，一台顯示本機、一台顯示 Mac](assets/screenshots/switch-center.png)
...
把會一起使用的螢幕與電腦編成一組。螢幕或電腦那一半都不勾選，就代表那一半全部涵蓋。群組只存在這台電腦，不會同步給已配對的主機。

![MuxSU 設定的群組分頁，兩個群組各自涵蓋一台螢幕](assets/screenshots/settings-groups.png)
