...
**螢幕維護（重啟螢幕與重送訊號為實驗性功能）**

- 「重啟螢幕」與「重送訊號」會寫入螢幕，目前只在 MSI MPG 274U 上驗證過，能不能用取決於螢幕韌體；失敗時可能需要用螢幕按鍵、或在另一台電腦上把畫面切回來。「重新偵測」只做讀取，不屬於實驗性。

- 畫面沒回來時，可以在設定頁對每一台共用螢幕直接重啟螢幕：透過 DDC/CI 關掉再開，不更動任何輸入設定。
- 也可以把螢幕切到另一台主機的輸入再切回來，讓它重新接上這台電腦。畫面離開之後只有當下在畫面上的那台主機能把它切回來，所以 MuxSU 會先確認對方有回應才動手，沒有回應就完全不動螢幕。
- 重新偵測會再向螢幕讀一次它接受哪些輸入、目前顯示哪一個；讀不到時保留原本的設定。
- 這些指令能不能用由螢幕韌體決定。重啟螢幕前會先讀螢幕宣告的電源指令，螢幕若沒有宣告開機指令就不會把它關掉（MSI MPG 274U 就是這種）；失敗時會說明螢幕停在哪裡、要怎麼把它拉回來。
- 螢幕內建的 USB／KVM 綁在「目前作用中的輸入」上，不是綁在面板電源。重啟螢幕不會把它帶回來，「重送訊號」會：MSI MPG 274U 實測，畫面與鍵盤滑鼠都會一起回到這台電腦。
