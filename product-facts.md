...
- 2026-09-22 實機讀取（capabilities string，純讀取取得）：
  - MSI MPG 274U E16M：`...C8 C9 D6(05) DC(00 02 03 05) DF FD)mccs_ver(2.1)`——VCP `0xD6` **只宣告 `0x05`**，既不接受 `0x04`，也**沒有開機指令 `0x01`**。這解釋了為什麼送出的電源指令毫無反應：值不在它接受的清單裡。若真的送 `0x05`，面板會關閉且無法用 DDC/CI 叫醒。
  - Acer VG252Q：`...C8 C9 CC(...) D6(01 04 05) DF E0(...)mccs_ver(2.2)`——三個值都宣告，`0x04` → `0x01` 的電源循環在這台上應可運作（尚未實測）。
...
- 因此截至 2026-09-22，VCP `0xD6` 電源循環在手上兩台螢幕都無法使用：MPG 274U 忽略指令，VG252Q 接受指令卻回不來。宣告支援不等於可用，這個功能不得以「部分螢幕可用」的說法出貨。
- 因此電源循環前必須先讀 `0xD6` 的宣告值：沒有宣告 `0x01` 就不送任何指令（送了也只是把螢幕關到只能按實體電源鍵），有宣告時則選它宣告過、且醒得回來的最深狀態（`0x04` → `0x03` → `0x02`）。capabilities 讀不到或沒宣告 `0xD6` 時維持原本的嘗試，因為沉默不是證據。
...
- 截至 2026-09-22 仍未有任何一台螢幕實機驗證過完整的電源循環：MPG 274U 不可能做到，Acer VG252Q 宣告支援但尚未測試。介面與文件不得宣稱這個功能可用。
